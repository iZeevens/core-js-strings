...
function countVowels(str) {
  return str && str.match(/[aeiouy]/gi) !== null
...
function isPalindrome(str) {
  const strReturn = str.split('').reverse().join('');
  return (
    strReturn.replace(/[\s\W]/g, '').toLowerCase() ===
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arraySentence.length; i += 1) {
    if (arraySentence[i].length > longSentence.length) {
      longSentence = arraySentence[i];
    }
  }
...
function reverseWords(str) {
  ...
  str.split(' ').forEach((elem) => {
    newArray.push(elem.split('').reverse().join(''));
  });
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((elem) => {
    if (/[a-m]/gi.test(elem)) {
      newArrUniCode.push(elem.charCodeAt() + 13);
    } else if (/[n-z]/gi.test(elem)) {
      newArrUniCode.push(elem.charCodeAt() - 13);
    } else {
      newArrUniCode.push(elem.charCodeAt(' '));
    }
  });

  return String.fromCodePoint(...newArrUniCode);
}
...
function getCardId(value) {
  const suitSymbol = value.slice(-1);
  const numberSumbol = value.slice(0, -1);
  let type = 0;
  let number = Number(numberSumbol);
  ...
  return type === 0 ? number - 1 : number - 1 + 13 * type;
}
